...
impl TestConnect {
    ...
    async fn oracle_query_table(&self, query: &str) -> Result<Vec<String>, oracle::Error> {
    ...
    async fn oracle_query_column(&self, query: &str) -> Result<Vec<String>, oracle::Error> {
        let pool = PoolBuilder::new(self.user.as_str(), self.passwd.as_str(), format!("//{}:1521/{}", self.host, self.database).as_str())
            .max_connections(10)
            .build();
        let result: Vec<String> = match pool {
    ...
    pub async fn oracle(&self) -> Result<String, oracle::Error> {
        let tables = self.oracle_query_table("SELECT table_name FROM user_tables").await?;
        // let mut columns: Vec<Vec<String>>;
        for i in tables {
            let column = self.oracle_query_column(format!("DESC {}", i).as_str()).await;
            println!("{:?}", column);
        }
        Ok("Hello".to_string())
    }
